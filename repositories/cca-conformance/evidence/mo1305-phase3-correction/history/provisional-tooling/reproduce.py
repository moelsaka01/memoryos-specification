...
R=Path.cwd();T=R/'repositories/cca-conformance/tools/mo1305-phase3-correction';E=R/'repositories/cca-conformance/evidence/mo1305-phase3-correction';sys.path.insert(0,str(T))
...
for label in ['clean-a','clean-b']:
 ...
 result=subprocess.run(command,capture_output=True,timeout=60);assert result.returncode==0,result.stderr.decode();print(result.stdout.decode().strip());outputs.append(out)
...
final=R/'.cache/mo1305-phase3-correction/build';final.mkdir()
...
(E/'reproducibility.json').write_bytes(j({'state':'PASS','independentCleanRoots':True,'separateBuilderProcesses':True,'byteIdentical':True,'assemblies':[{'path':str(x.relative_to(R))+'/memoryos-rest-0.1.0.tgz',**identity((x/'memoryos-rest-0.1.0.tgz').read_bytes())} for x in outputs],'sourceTree':identity(j(tree)),'archive':{'path':str((final/'memoryos-rest-0.1.0.tgz').relative_to(R)).replace('\\','/'),**identity(a)}}))
