/** Fresh staged probe. Imports only Node builtins and verified installed archive members. */
...
const load = member => {
  const target = resolve(packagePath, member);
  assert.ok(relative(packagePath, target) && !relative(packagePath, target).startsWith('..' + sep) && !isAbsolute(relative(packagePath, target)));
  return import(pathToFileURL(target).href);
};
const record = {kind: 'MemoryOSRESTPhase3BRInstalledProbe', version: '1.0.0', state: 'RUNNING', requests: []};
let child = null;
...
function request(route, input, configuration, token, ca, api, requestId, {headers: overrides={}, expectedStatus=200}={}) {
  ...
  return new Promise((resolveResponse, reject) => {
    ...
    const timer = setTimeout(() => {failure = Error('REQUEST_TIMEOUT'); socket.destroy();}, 15000);
...
try {
  ...
  const sbomBytes=readFileSync(resolve(packagePath,'sbom.spdx.json'));
  const sbom=JSON.parse(sbomBytes);
  assert.deepEqual(identity(sbomBytes),{byteLength:44094,sha256:'ab0a60fc4273390fe353df5c8464571b581043c2ddc90587039afa1299ad3b7b'});
  assert.equal(sbom.packages.length,25);assert.equal(sbom.files.length,56);assert.equal(sbom.relationships.length,81);
  const closure=JSON.parse(readFileSync(resolve(packagePath,'runtime/runtime-closure-manifest.json')));
  assert.equal(closure.files.length,25);
  record.deployment=api.deployment;
  record.sbom={...identity(sbomBytes),packages:25,files:56,relationships:81,distributionIntegrity:'PASS',structuralValidationScope:'external pinned Python validator cheap gate'};
  record.verification = {runtime: 'PASS', distribution: 'PASS', contracts: 'PASS', schemas: 'PASS',
    openapi: 'PASS', limitsState: limits.state, semanticClosureFileCount: closure.files.length, distributionManifestSha256: manifestSha256};
  ...
  for (const item of cases) {
    ...
    const response = await request(route, item.input, configuration, token, ca, api, `phase3br-${item.id}`);
  ...
  for(const [label,headers,status,code] of [['missing-auth',{Authorization:null},401,'MO1305_UNAUTHENTICATED']]){
    const value=await request(api.routes.find(r=>r.operationId==='getHealth'),null,configuration,token,ca,api,'phase3br-'+label,{headers,expectedStatus:status});
  ...
  record.scope = 'bounded loopback TLS execution; remote deployment metadata verified without repeating Windows certification';
  record.state = 'PASS';
} catch (error) {
  record.state = 'FAIL';
