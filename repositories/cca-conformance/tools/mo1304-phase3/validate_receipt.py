"""Strict, offline Ubuntu-only evidence validator. No cross-platform PASS path."""
...
SOURCE_SHA='b170599880cbd73f8eb6a5800c08568a361e256d4fccc0d30ceac6eb4e3a99f4'
PENDING={'windows11_24H2_x64':'PENDING','windowsReceipt':None,'crossPlatformParity':'PENDING','parityReceipt':None,'finalReleaseBinding':'PENDING','tag':'ABSENT','macos':'UNSUPPORTED'}
KEYS={'kind','version','platform','architecture','os','node','npm','implementationRevision','phase2Binding','candidate','harness','testCatalog','artifacts','semanticVectors','supplyChain','coverage','pending','overall','nonNormative'}
ARTIFACT_KEYS={'execution','packageAdversarial','mechanismsStdout','mechanismsStderr','inputs','supplyChainSources','installation','harnessManifest','testCatalog','toolingManifest','toolingVerification'}
...
def validate(value,directory):
 ...
 require(value['kind']=='MemoryOSMO1304UbuntuCertification' and value['version']=='1.0.0','SCHEMA')
 require(value['platform']=='ubuntu-24.04' and value['architecture']=='x64','PLATFORM')
 require(set(value['os'])=={'prettyName','versionId','kernel'} and value['os']['prettyName']=='Ubuntu 24.04.5 LTS' and value['os']['versionId']=='24.04' and re.fullmatch(r'[0-9A-Za-z.+_-]{1,128}',value['os']['kernel']) is not None,'OS')
 ...
 execution=read_json(paths['execution'][0]);require(execution['kind']=='MemoryOSMO1304UbuntuExecutionAttempt' and execution['status']=='PASS_EXECUTED_SUBSET','ACTUAL_EXECUTION')
 ...
 require(by_id['os-network-denial']['details']['probes']==[[2,1,1],[10,1,1],[2,2,1]],'OS_DENIAL_PROBES')
 audit=by_id['installed-filesystem-and-supply-chain-boundary']['details']['audit']
 require(audit['code']==0 and len(audit['workers'])==16,'BOUNDARY_WORKERS')
 ...
 for key in [r['id'] for r in results if r['id'].startswith('maximal-')]:
  d=by_id[key]['details'];require(d['attributablePeak']<=500170752 and d['frozenParentBound']==500170752 and d['durationMs']<20000,'RESOURCE_BOUND')
 package=read_json(paths['packageAdversarial'][0]);require(package['kind']=='MemoryOSMO1304UbuntuPackageAdversarial' and package['status']=='PASS','PACKAGE_VERIFIER')
 ...
 require(package['archive']==value['candidate']['archive'] and package['filesystemPlatform']=='linux' and package['windowsReparseOrADSClaimed'] is False,'PACKAGE_TEST_PLATFORM')
 ...
 install=read_json(paths['installation'][0]);require(install['exitCode']==0 and install['networkDenied'] is True and install['verified']['archiveFiles']==798,'OFFLINE_INSTALL')
 cmd=install['command'];require(cmd[0]=='/opt/memoryos-cert/node-v24.21.0-linux-x64/bin/node' and cmd[1]=='/opt/memoryos-cert/node-v24.21.0-linux-x64/lib/node_modules/npm/bin/npm-cli.js','INSTALL_TOOLCHAIN')
 require(all(x in cmd for x in ['--offline','--ignore-scripts','--no-audit','--no-fund','--no-save','--omit=dev','--cache','--prefix']) and cmd[-1]=='/home/mo1304/mo1304-cert/incoming/'+ARCHIVE,'INSTALL_COMMAND')
 ...
 require(manifest['kind']=='MemoryOSMO1304UbuntuHarnessManifest' and manifest['version']=='1.0.0','HARNESS_MANIFEST')
 names=[f['path'] for f in manifest['files']];require(names==sorted(set(names)) and names==sorted(['allocation_worker.mjs','boundary_guard.mjs','boundary_process.mjs','boundary_worker.mjs','certify_ubuntu.py','idle_worker.mjs','installed_boundaries.mjs','linux_network_deny.py','package_adversarial.py','package_verify.py','receipt.schema.json','test-catalog.json','ubuntu_driver.mjs','validate_receipt.py']),'HARNESS_FILE_SET')
 ...
 return {'status':'PASS','platform':'ubuntu-24.04','nativePipeTests':len(results),'packageTests':len(package['results']),'mechanismTests':catalog['supplementalInstalledModuleTests'],'windows':'PENDING','parity':'PENDING'}
