/** Installed-package Ubuntu certification over actual OS pipes. */
...
for(const p of [root,tooling,inputPath,evidence])assert.ok(p?.startsWith('/home/mo1304/mo1304-cert/'));
assert.equal(process.execPath,'/opt/memoryos-cert/node-v24.21.0-linux-x64/bin/node');assert.equal(process.versions.node,'24.21.0');assert.equal(process.platform,'linux');assert.equal(process.arch,'x64');
...
const record={kind:'MemoryOSMO1304UbuntuExecutionAttempt',version:'1.0.0',status:'RUNNING',implementation:input.implementation,binding:input.binding,inputSha256:sha(inputBytes),node:process.versions.node,nodeExecutableSha256:sha(await readFile(process.execPath)),results,vectors};
...
async function rss(pid){try{return Number((await readFile(`/proc/${pid}/status`,'utf8')).match(/^VmRSS:\s+(\d+)/m)?.[1]??0)*1024;}catch{return 0;}}
async function official(launch,environment={}){
 ...
 const client=new Client({name:'memoryos-ubuntu-certification',version:'1.0.0'},{capabilities:{},versionNegotiation:{mode:{pin:'2026-07-28'}},inputRequired:{autoFulfill:false}});
...
try{
 await test('os-network-denial',async()=>{
  const r=spawnSync('/usr/bin/python3',['-c','import socket,errno,json; r=[]\nfor f,k in [(socket.AF_INET,socket.SOCK_STREAM),(socket.AF_INET6,socket.SOCK_STREAM),(socket.AF_INET,socket.SOCK_DGRAM)]:\n try: socket.socket(f,k); raise RuntimeError("NO_DENIAL")\n except OSError as e: assert e.errno==errno.EPERM; r.append([f,k,e.errno])\nprint(json.dumps(r))'],{encoding:'utf8',env:{},timeout:5000});assert.equal(r.status,0);assert.equal(r.stderr,'');return {mechanism:'Linux libseccomp inherited process-tree filter, EPERM for socket/connect/bind/listen/accept/accept4/sendto/sendmmsg/io_uring_setup',probes:JSON.parse(r.stdout),sshManagementOutsideFilter:true};
 });
 ...
 await test('installed-filesystem-and-supply-chain-boundary',async()=>{
  const auditPath=resolve(evidence+'.boundary.json'),bootstrap=resolve(new URL('.',import.meta.url).pathname,'boundary_process.mjs');
  ...
  try{
   ...
   for(const path of ['/etc/passwd','../secret','/home/mo1304/.ssh/authorized_keys','/workspace/cca-workspace/private','file:///etc/passwd','http://127.0.0.1/private','http://host:80@private/']){
  ...
  return {instrumentation:'Trusted explicit bootstrap around unchanged installed parent and workers, supplemental to uninstrumented real-pipe semantic runs',osDenial:'Inherited Linux seccomp filter also active',workerOperations:operationCount,audit,windowsSpecificTestsClaimed:false};
 });
 ...
 await test('runtime-debug-injection',async()=>{
  ...
  return {exitCode:1,productDiagnosticBytes:13,nodePreEntryDiagnosticBytes:Buffer.byteLength(r.stderr)-13,networkFilterInherited:true,scope:'Unsupported debugger startup is rejected. Node may emit pre-entry diagnostics before the fixed product fatal line; no hostile-administrator protection is claimed.'};
 });
 ...
 await test('native-repeated-worker-cleanup-memory',async()=>{
  const h=await official(),samples=[];const threads=async()=>Number((await readFile(`/proc/${h.pid}/status`,'utf8')).match(/^Threads:\s+(\d+)/m)[1]);
  try{
