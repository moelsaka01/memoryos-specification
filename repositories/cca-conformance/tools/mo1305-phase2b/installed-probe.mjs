...
const record = {kind: 'MemoryOSRESTPhase2BInstalledProbe', version: '1.0.0', state: 'RUNNING', requests: []};
let child = null;
...
function request(route, input, configuration, token, ca, api, requestId) {
  const body = input === null ? '' : JSON.stringify(input);
  const fields = {Host: `127.0.0.1:${configuration.port}`, Authorization: `Bearer ${token}`,
    Connection: 'close', 'Accept-Encoding': 'identity', 'X-Request-ID': requestId};
  ...
  return new Promise((resolveResponse, reject) => {
    const socket = tls.connect({host: '127.0.0.1', port: configuration.port, ca,
    ...
    socket.once('close', () => {
      ...
      try {
        ...
        assert.equal(status, 200); assert.equal(Number(headers['content-length']), responseBody.length);
        ...
        resolveResponse({body: JSON.parse(responseBody), receipt: {operationId: route.operationId, status,
          ...
          requestHeaders: Object.fromEntries(Object.entries(fields).map(([key, value]) => [key,
            key === 'Authorization' ? 'Bearer <REDACTED>' : key === 'Host' ? '127.0.0.1:<ISOLATED_PORT>' : value]))}});
...
try {
  ...
  for (const item of cases) {
    ...
    const response = await request(route, item.input, configuration, token, ca, api, `phase2b-${item.id}`);
