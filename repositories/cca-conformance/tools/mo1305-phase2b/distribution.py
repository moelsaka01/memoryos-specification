"""Phase 2B trusted, bounded distribution builder/verifier (stdlib only).

Run from trusted engineering media before any installed JavaScript. The expected
archive digest must come from an independently trusted Phase 2B receipt. This is
...
def import_graph(files):
    ...
    for name, content in sorted(files.items()):
        ...
        # Every executable byte is independently B1-pinned above.
...
def verify_files(files):
    ...
    for row in POLICY['immutable']:
        require(ref(row['path'], files[row['path']]) == row, 'B1_IMMUTABLE:' + row['path'])
    ...
    for row in sbom['files']:
        ...
        require(row['checksums'] == [{'algorithm': 'SHA256', 'checksumValue': sha(files[row['fileName'][2:]])}], 'SBOM_FILE_CHECKSUM')
        ids.add(row['SPDXID'])
...
def assemble(root, node, npm):
    ...
    metadata(files)
    for row in POLICY['immutable']:
        require(ref(row['path'], files[row['path']]) == row, 'B1_BUILD_DRIFT:' + row['path'])
    inputs = [ref(PKG_REL + '/' + name, data) for name, data in sorted(files.items())]
    inputs += [ref('repositories/cca-conformance/tools/mo1305-phase2b/' + path.name, checked_read(path))
               for path in sorted(HERE.iterdir()) if path.suffix in ('.py', '.mjs', '.json')]
    support = ['repositories/cca-conformance/tools/mo1305-phase1/validate-launch.ps1',
    ...
    for index, (name, data) in enumerate(sorted(files.items())):
        ...
        file_rows.append({'SPDXID': spdx, 'fileName': './' + name, 'checksums': [{'algorithm': 'SHA256', 'checksumValue': sha(data)}],
    ...
    files['sbom.spdx.json'] = j({'spdxVersion': 'SPDX-2.3', 'dataLicense': 'CC0-1.0', 'SPDXID': 'SPDXRef-DOCUMENT',
        ...
        'creationInfo': {'creators': ['Tool: memoryos-rest-phase2b-builder-1.0.0'], 'created': '2026-09-25T00:00:00Z'},
