"""Bounded Phase 2B installation and execution with copied tools outside the checkout.
...
def install(args):
    ...
    stage = Path(tempfile.mkdtemp(prefix='memoryos-rest-phase2b-', dir=temp_parent)).resolve()
    assert stage.parent == temp_parent and not stage.is_relative_to(ROOT), 'ISOLATION_DIRECTORY'
    record = {'kind': 'MemoryOSRESTPhase2BInstalledValidation', 'version': '1.0.0', 'state': 'RUNNING',
...
''', encoding='utf-8')
        ...
        assert cleanup_target == stage and cleanup_target.parent == temp_parent and cleanup_target.name.startswith('memoryos-rest-phase2b-'), 'CLEANUP_SCOPE'
...
if __name__ == '__main__':
    ...
    try:
        ...
        print('Phase 2B installed validation failed; inspect the bounded receipt.', file=sys.stderr)
        sys.exit(1)
