"""Phase 2D trusted, bounded distribution builder/verifier (stdlib only).

Run from trusted engineering media before any installed JavaScript. The expected
archive digest must come from an independently trusted Phase 2D receipt. This is
...
def assemble(root, node, npm):
    ...
    inputs += [ref('repositories/cca-conformance/tools/mo1305-phase2d/' + path.name, checked_read(path))
               for path in [HERE / name for name in ('distribution.py', 'package-allowlist.json')]]
    ...
    files['sbom.spdx.json'] = j({'spdxVersion': 'SPDX-2.3', 'dataLicense': 'CC0-1.0', 'SPDXID': 'SPDXRef-DOCUMENT',
        ...
        'creationInfo': {'creators': ['Tool: memoryos-rest-phase2d-builder-1.0.0'], 'created': '2026-09-25T00:00:00Z'},
