"""Bounded Phase 2D installation and execution with copied tools outside the checkout.
...
FIXTURES = ('identities.json', 'prepare-policy-pass.json', 'prepare-policySet-pass.json',
            'evaluate-policy-pass.json', 'evaluate-policy-fail.json', 'evaluate-policy-cne.json', 'verify-identity.json', 'verify-outcome.json')
...
def install(args):
    ...
    stage = Path(tempfile.mkdtemp(prefix='memoryos-rest-phase2d-', dir=temp_parent)).resolve()
    assert stage.parent == temp_parent and not stage.is_relative_to(ROOT), 'ISOLATION_DIRECTORY'
    record = {'kind': 'MemoryOSRESTPhase2DInstalledValidation', 'version': '1.0.0', 'state': 'RUNNING',
              'scope': 'bounded packaging and installed execution; not Phase 2C or Phase 3 certification',
    ...
    try:
        ...
        assert not list((stage / 'ci-cache').iterdir()), 'NONEMPTY_CI_CACHE'
        command([tool_node, tool_npm, 'ci', *npm_flags, '--cache', stage / 'ci-cache'], extracted, env=npm_env)
        package_inventory(extracted, expected_inventory)
        ...
        record['offline'] = {'ciCacheInitiallyEmpty': True, 'installCacheInitiallyEmpty': True,
            ...
            'installedPackageMatchesArchive': True, 'ciCacheAfter': inventory(stage / 'ci-cache'),
            'installCacheAfter': inventory(stage / 'install-cache')}
        import ipaddress
...
''', encoding='utf-8')
        ...
        refusal = command(trusted_args, stage / 'empty', env={**clean, 'NODE_PATH': str(stage / 'untrusted-source')}, expected=2)
        assert refusal.stdout.strip() == b'MO1305_TRUSTED_LAUNCH_REFUSED:ENVIRONMENT', 'SOURCE_ENVIRONMENT_NOT_REFUSED'
        fake_node = stage / 'substituted-node.exe'
        fake_node.write_bytes(b'not the trusted Node executable')
        fake_args = [fake_node if str(item) == str(tool_node) else item for item in trusted_args]
        refusal = command(fake_args, stage / 'empty', expected=2)
        assert refusal.stdout.strip() == b'MO1305_TRUSTED_LAUNCH_REFUSED:RUNTIME', 'NODE_SUBSTITUTION_NOT_REFUSED'
        gateway_refusal = command([tool_node, installed / 'bin/memoryos-rest.mjs', '--version'], stage / 'empty',
                                  env={**clean, 'NODE_PATH': str(stage / 'untrusted-source')}, expected=2)
        assert gateway_refusal.stdout == b'', 'REFUSAL_STDOUT'
        assert json.loads(gateway_refusal.stderr) == {'code': 'MO1305_UNAVAILABLE', 'event': 'fatal', 'operationId': None, 'requestId': None}, 'GATEWAY_ENVIRONMENT_REFUSAL'
        record['trustedLauncherNegatives'] = {'environmentSelectedSource': 'REFUSED', 'nodeSubstitution': 'REFUSED',
                                              'gatewayEnvironmentSelectedSource': 'REFUSED_BEFORE_STARTUP'}
        for label, package in [('direct-extraction', extracted), ('npm-offline-install', installed)]:
        ...
        assert cleanup_target == stage and cleanup_target.parent == temp_parent and cleanup_target.name.startswith('memoryos-rest-phase2d-'), 'CLEANUP_SCOPE'
...
if __name__ == '__main__':
    ...
    try:
        ...
        print('Phase 2D installed validation failed; inspect the bounded receipt.', file=sys.stderr)
        sys.exit(1)
