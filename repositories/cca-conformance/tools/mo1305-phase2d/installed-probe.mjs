...
const record = {kind: 'MemoryOSRESTPhase2DInstalledProbe', version: '1.0.0', state: 'RUNNING', requests: []};
let child = null;
...
try {
  ...
  for (const item of cases) {
    ...
    const response = await request(route, item.input, configuration, token, ca, api, `phase2d-${item.id}`);
  ...
  assert.equal(new Set(record.requests.map(item => item.operationId)).size, 9);
  for(const [label,headers,status,code] of [['missing-host',{Host:null},403,'MO1305_FORBIDDEN'],['missing-host-and-auth',{Host:null,Authorization:null},401,'MO1305_UNAUTHENTICATED']]){
    const value=await request(api.routes.find(r=>r.operationId==='getHealth'),null,configuration,token,ca,api,'phase2d-'+label,{headers,expectedStatus:status});
  ...
  for(const operation of ['getHealth','getReadiness','getContractIdentities']){
    const item=cases.find(x=>x.operation===operation);assert.ok(item);
    const value=await request(api.routes.find(r=>r.operationId===operation),item.input,remoteConfig,token,ca,api,'phase2d-remote-'+operation);
    assert.deepEqual(value.body,item.expected);remoteRequests.push({case:operation,...value.receipt,expectedBodyMatched:true});
  }
  for(const [label,headers,status,code] of [['missing-host',{Host:null},403,'MO1305_FORBIDDEN'],['missing-host-and-auth',{Host:null,Authorization:null},401,'MO1305_UNAUTHENTICATED']]){
    const value=await request(api.routes.find(r=>r.operationId==='getHealth'),null,remoteConfig,token,ca,api,'phase2d-remote-'+label,{headers,expectedStatus:status});
