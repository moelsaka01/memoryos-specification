/** Separate exact-Node fetch process. Test credentials arrive only on stdin. */
import assert from 'node:assert/strict';
let input = '';
for await (const chunk of process.stdin) {
  input += chunk;
  assert.ok(Buffer.byteLength(input) <= 1048576, 'FETCH_INPUT_BOUND');
}
...
for (const item of cases) {
  assert.match(item.path, /^\/v1\/[a-z/-]+$/u);
  assert.ok(['GET', 'POST'].includes(item.method));
  ...
  for await (const chunk of response.body) {
    ...
    if (bytes.length > 65536) throw Error('FETCH_RESPONSE_BOUND');
  }
  records.push({id: item.id, status: response.status,
    headers: Object.fromEntries(response.headers), bodyBase64: bytes.toString('base64')});
