...
import {put as immutablePut,base as evidenceBase} from './mo1305-modular-resource.mjs';
import {clock,monoMs,elapsedNs,wallObservation,failureEvidence,relation,protocolVersion} from './mo1305-phase1/clock.mjs';
import {projectBarrier} from './mo1305-phase1/sample-proof.mjs';
...
import {trustedValidationBroker,launch,stage,root,credentials,request,response,wire,delay,vectors} from './mo1305-phase1/installed.mjs';
import {monitor,osSummary,stateSummary} from './mo1305-phase1/monitor.mjs';import {J} from '../../memoryos-rest/src/serialization.mjs';
...
const full=plan.layer==='stress'||plan.layer==='stress-confirmation',duration=plan.durationMs,repetitions=plan.repetitions;
...
async function coreExecute(name,repeat){
 ...
 async function start(){current=await launch({validationBroker,measurement:true,args:name.includes('published')||name==='blocked-writers-4'?[resolve(import.meta.dirname,'mo1305-phase1/write-fault-server.mjs'),stage]:null});current.child.on('message',value=>{if(value.type==='sample'){allSamples.push({...value.record,operationTiming:value.operationTiming??null,provenance:{campaignId,sessionId:os.identity.sessionId,pid:current.child.pid,observed:clock.stamp()}});current.observations.length=0;}});os=await monitor(current.child,resolve(rawOutput,name+'-'+repeat+'-'+activeAttempt+'-'+counts.cycles+'.json'),{candidateId:binding.sourceTreeSha256,archiveSha256:binding.archive.sha256,campaignId});pre=await os.barrier();post=null;}
 ...
 try{
  ...
  const bytes=Buffer.from(J(record));assert.ok(bytes.length<=2097152);writeFileSync(resolve(output,name+'-'+repeat+'-'+activeAttempt+'.json'),bytes);console.log(JSON.stringify({name,repeat,state:'PASS',adverseElapsedMs,requests:counts.requests,peak:parent.peak}));return record;
 }catch(error){const failure=failureEvidence(error,{case:name,phase:'adverse',index:repeat,candidateId:binding.sourceTreeSha256,campaignId,runId:os?.identity.sessionId??null,pid:current?.child.pid??null,captureTicket:os?.rows.at(-1)?.ticket??null,sequence:os?.rows.at(-1)?.sequence??null,pre,post,observed:clock.stamp(),counts,cycles,runs,operationSamples:allSamples});writeFileSync(resolve(output,name+'-'+repeat+'-'+activeAttempt+'-failed.json'),J(failure));try{if(current)await cleanup();}catch(cleanupError){writeFileSync(resolve(output,name+'-'+repeat+'-'+activeAttempt+'-cleanup-failed.json'),J(failureEvidence(cleanupError,{case:name,phase:'adverse',index:repeat})));}throw error;}
}

let activeAttempt=null;const hostAttempts=[];
async function execute(name,repeat){
 for(let attempt=0;attempt<=3;attempt++){
  activeAttempt=randomUUID();const start=clock.stamp();let record=null,error=null;
  try{record=await coreExecute(name,repeat);}catch(e){error=e;}
  const end=clock.stamp(),events=await queryEvents(start.utcMs,end.utcMs),eventRef=immutablePut(evidenceBase+'/host-events/'+activeAttempt+'.json',events);
  const guard=classify({start,end,operationInterval:record?.timing??null,attemptId:campaignId,candidateId:binding.sourceTreeSha256,caseId:name,phase:plan.layer,index:repeat,sampleId:activeAttempt},events);
  const row={sampleId:activeAttempt,name,repeat,attempt,guard,hostEvents:eventRef,record:record?identity(resolve(output,name+'-'+repeat+'-'+activeAttempt+'.json')):null,failure:error?{message:error.message,code:error.code??null}:null};
  hostAttempts.push(immutablePut(evidenceBase+'/stress-observations/'+activeAttempt+'.json',row));writeFileSync(resolve(output,'host-attempts.json'),J(hostAttempts));
  if(guard.classification==='HOST_INTERRUPTED'){assert.ok(replacementAllowed(attempt+1),'HOST_ENVIRONMENT_UNSTABLE');continue;}
  if(error)throw error;
  writeFileSync(resolve(output,name+'-'+repeat+'.json'),J(record),{flag:'wx'});writeFileSync(resolve(output,name+'-'+repeat+'-host.json'),J(row),{flag:'wx'});return record;
 }throw Error('HOST_ENVIRONMENT_UNSTABLE');
}
