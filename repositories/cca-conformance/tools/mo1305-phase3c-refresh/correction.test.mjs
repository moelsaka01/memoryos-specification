// Original eight correction tests replayed with explicit refresh context; no additional coverage count.
...
test('current package, full SPDX, receipts, runtime preservation and correction graph pass',()=>{const r=run(['repositories/cca-conformance/tools/mo1305-phase3c-refresh/context.py']);assert.equal(r.state,'PASS');assert.equal(r.fileCount,58);assert.equal(r.runtimeUnchanged,52);assert.equal(r.metadataCases,34);});
