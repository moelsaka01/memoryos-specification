import {classify,queryEvents,replacementAllowed} from './mo1305-host-guard.mjs';
import {unchangedInputs,ref as modularRef,put as immutablePut,base as evidenceBase} from './mo1305-modular-resource.mjs';
import assert from 'node:assert/strict';
import {readFileSync,writeFileSync,mkdirSync,existsSync} from 'node:fs';
import {resolve} from 'node:path';
import {createHash,randomUUID} from 'node:crypto';
import {trustedValidationBroker,launch,root,stage,response,wire,delay,api} from './mo1305-phase1/installed.mjs';
import {catalog,raw,fixture} from './mo1305-phase1/campaign-catalog.mjs';
import {monitor,osSummary,stateSummary} from './mo1305-phase1/monitor.mjs';
import {validateWindow,validateChunk,makeWindow} from './mo1305-phase1/sample-proof.mjs';
import {clock,elapsedUs,failureEvidence,protocolVersion,relation} from './mo1305-phase1/clock.mjs';
...
assert.ok(plan.output.startsWith('repositories/cca-conformance/evidence/mo1305-phase1-resume/')&&!plan.output.includes('..'));
...
const rawOutput=resolve(root,'.cache/mo1305-host-resume',plan.layer+'-raw');mkdirSync(rawOutput,{recursive:true});
...
async function coreSample(item,run,phase,index){
 ...
 }catch(error){const failed=failureEvidence(error,{...context,captureTicket:error.capture?.requestedTicket??os.rows.at(-1)?.ticket??null,sequence:error.capture?.lastSequence??os.rows.at(-1)?.sequence??null,captureContext:error.capture??null,observed:clock.stamp(),observations:state.observations,parent:state.states.length?stateSummary(state.states):null,native:os.rows.length?osSummary(os.rows):null});writeFileSync(resolve(output,item.id+'-'+activeSampleId+'-failed-sample.json'),J(failed));throw error;}
}
const retained=JSON.parse(readFileSync(resolve(root,evidenceBase,'r6-reuse.json')));unchangedInputs();assert.deepEqual(binding,retained.inputs.binding);
let activeSampleId=null;const attempts=[],accepted=[],slotCounts=new Map(),vectorCounts=new Map();
for(const old of retained.invalid){const key=old.case+':'+old.guard.phase+':'+old.guard.index;slotCounts.set(key,(slotCounts.get(key)??0)+1);vectorCounts.set(old.case,(vectorCounts.get(old.case)??0)+1);}
async function guarded(item,run,phase,index){
 activeSampleId=randomUUID();const started=clock.stamp();let record=null,error=null;
 try{record=await coreSample(item,run,phase,index);}catch(e){error=e;}
 const context=activeContext,end=context.post?.received??clock.stamp(),start=context.pre?.cutoff??started,events=await queryEvents(start.utcMs,end.utcMs);
 const eventRef=immutablePut(evidenceBase+'/host-events/'+activeSampleId+'.json',events);
 const guard=classify({start,end,operationInterval:record?.operationTiming??context.validSample?.operationTiming??run.state.observations[0]?.operationTiming??null,attemptId:campaignId,candidateId:binding.sourceTreeSha256,caseId:item.id,phase,index,sampleId:activeSampleId},events);
 const observation={sampleId:activeSampleId,case:item.id,phase,index,hostEvents:eventRef,guard,record:record??context.validSample??null,failure:error?{message:error.message,code:error.code??null}:null};attempts.push(immutablePut(evidenceBase+'/observations/'+activeSampleId+'.json',observation));
 if(guard.classification==='HOST_INTERRUPTED'){const key=item.id+':'+phase+':'+index,n=(slotCounts.get(key)??0)+1,v=(vectorCounts.get(item.id)??0)+1;slotCounts.set(key,n);vectorCounts.set(item.id,v);assert.ok(replacementAllowed(n,v),'HOST_ENVIRONMENT_UNSTABLE');return null;}
 if(error)throw error;accepted.push({case:item.id,phase,index,observation:attempts.at(-1)});return record;
}
try{
 for(const item of cases){
  const slots=retained.missing.filter(s=>s.case===item.id),records=[],runs=[];
  const persist=()=>writeFileSync(resolve(output,item.id+'.json'),J({campaignId,runs,id:item.id,fixtureSha256:fixture(item),complete:records.length===slots.length,records,accepted:accepted.filter(x=>x.case===item.id)}));
  let position=0;
  while(position<slots.length){const slot=slots[position];let run=null,error=null;const runId=randomUUID();
   try{run=await start(item,slot.phase+'-'+slot.index+'-'+runId);do{const current=slots[position],record=await guarded(item,run,current.phase,current.index);if(!record)break;records.push(record);position++;persist();}while(slot.phase==='warm'&&position<slots.length&&slots[position].phase==='warm');}
   catch(e){error=e;}
   finally{if(run){let exit=null,cleanupError=null;try{exit=await run.state.stop();}catch(e){cleanupError=e;}try{await run.os.stop();}catch(e){cleanupError??=e;}const {sessionId,pid,collectorPid,rawPath,candidateId,archiveSha256,campaignId}=run.os.identity;const raw=existsSync(rawPath)?identity(rawPath):null,creation=raw?JSON.parse(readFileSync(rawPath)).processStartedUtcMs:null;runs.push({sessionId,pid,collectorPid,candidateId,archiveSha256,campaignId,processStartedUtcMs:creation,phase:slot.phase,index:slot.index,exitCode:exit?.code??run.state.child.exitCode,collectorExitCode:cleanupError?null:0,raw});error??=cleanupError;}persist();}
   if(error)throw error;
  }
  progress.completed.push({id:item.id,freshSamples:records.length,samples:identity(resolve(output,item.id+'.json'))});save();console.log(JSON.stringify({case:item.id,state:'PASS',freshSamples:records.length,completed:progress.completed.length,total:cases.length}));
 }
 unchangedInputs();progress.state='PASS';progress.ended=clock.stamp();progress.attempts=attempts;progress.accepted=accepted;save();
}catch(error){progress.state='FAIL';progress.ended=clock.stamp();progress.failure={message:error.message,code:error.code??null,context:activeContext};progress.attempts=attempts;progress.accepted=accepted;save();throw error;}finally{await validationBroker.stop();}
