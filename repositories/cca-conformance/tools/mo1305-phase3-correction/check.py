...
H='repositories/cca-conformance/evidence/mo1305-phase3-correction/'
E=H+'accepted/'
...
def parallel_proof():
    for row in read(H+'parallel-inputs.json'):
...
def expected_inventory():
    inventory=read(H+'historical-inventory-b2.json');candidate=read(E+'candidate.json')
...
def binding(c3):
    ...
    return {'kind':'MemoryOSRESTReleaseMetadataCorrectionBinding','version':'1.0.0','baseline':BASELINE,'implementation':c3,'implementationSubject':C3_SUBJECT,'candidate':reference(E+'candidate.json'),'old':read(H+'baseline.json')['old'],'provisionalBlocked':read(H+'final/dirty-tree-audit.json')['provisional'],'schema':candidate['schema'],'corrected':candidate['corrected'],'archive':candidate['archive'],'runtimePreservation':reference(E+'runtime-preservation.json'),'evidence':candidate['evidence'],'phase3':candidate['phase3'],'finalBinding':'PENDING','releaseTag':'ABSENT'}

def check_binding(value,c3):
    assert value==binding(c3),'BINDING_CONTENT'
    return True
...
def graph(require_binding=False):
    ...
    if value:check_binding(value,c3)
    if head==c3:
...
def validate(require_binding=False,parallel=False):
    ...
    report={'state':'PASS','archive':verified['archive'],'fileCount':58,'runtimeUnchanged':52,'metadataCases':34,'contractTests':12,'graph':graph(require_binding)}
