...
def assemble(root, node, npm):
    ...
    inputs += [ref('repositories/cca-conformance/tools/mo1305-phase3-correction/' + path.name, checked_read(path))
               for path in [HERE / name for name in ('distribution.py', 'package-allowlist.json', 'spdx.py', 'schema_validation.py', 'schema-runtime.json')]]
    support = ['repositories/cca-conformance/evidence/mo1305-phase3-correction/spdx-2.3-schema.json',
               'repositories/cca-conformance/tools/mo1305-phase1/build.py',
    ...
    files['sbom.spdx.json'] = j({'spdxVersion': 'SPDX-2.3', 'dataLicense': 'CC0-1.0', 'SPDXID': 'SPDXRef-DOCUMENT',
        ...
        'comment': 'Gateway and authoritative closure files are distinct. SPDX and distribution manifest exclude self hashes; archive receipt binds both. Zero npm production dependencies does not mean zero supply-chain surface.',
