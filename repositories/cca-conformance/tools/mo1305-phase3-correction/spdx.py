...
import hashlib
import re
from schema_validation import verify_schema
...
def verify_spdx(document, files):
    ...
    for file in document.get('files', []):
        need(set(file) == {'SPDXID', 'fileName', 'checksums', 'licenseConcluded', 'licenseInfoInFiles', 'copyrightText'}, 'FILE_FIELDS')
        ...
        need(file['licenseConcluded'] == file['copyrightText'] == 'NOASSERTION' and file['licenseInfoInFiles'] == ['NOASSERTION'], 'FILE_LICENSE')
        names.add(name)
    ...
    for package in document.get('packages', []):
        ...
        if not package['filesAnalyzed']:
            need(not targets and not package.get('hasFiles'), 'FALSE_WITH_FILES:' + package['name'])
