...
The CLI is deliberately thin: it validates command input, invokes the public SDK, and renders deterministic human-readable or JSON output. All investigation state transitions, trace construction, replay, comparison, package verification, and checkpoint validity remain SDK-owned.
...
The stateless `replay` and `compare` commands require an exact `--trace` selector because the frozen SDK requires the Trace lifecycle transition before Replay. Comparison accepts one package containing authored Evolution data; the CLI never merges two packages.
