...
int main() {
    try {
        ...
        baselineOptions.identifier = "cpp-explorer-baseline";
        ...
        candidateOptions.identifier = "cpp-explorer-candidate";
        ...
        const auto result = memory.investigate(report, std::move(query));
        if (result.status() != "matched" || result.matchCount() == 0U) {
            return 1;
        }
        std::cout << result.canonicalJson() << '\n';
        return 0;
